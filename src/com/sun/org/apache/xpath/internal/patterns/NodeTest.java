...
/**
 * This is the basic node copy class for both match patterns and location path
 * steps.
 * @xsl.usage advanced
 */
public class NodeTest extends Expression
{
  ...
  /**
   * Statically calculated score for this copy.  One of
   *  {@link #SCORE_NODETEST},
   *  {@link #SCORE_NONE},
   *  {@link #SCORE_NSWILD},
   *  {@link #SCORE_QNAME}, or
  ...
  /**
   * Construct an NodeTest that doesn't copy for node names.
   *
   *
   * @param whatToShow Bit set defined mainly by {@link org.w3c.dom.traversal.NodeFilter}.
   */
  ...
  /**
   * Initialize this node copy by setting the whatToShow property, and
   * calculating the score that this copy will return if a copy succeeds.
   *
   *
   * @param whatToShow Bit set defined mainly by {@link org.w3c.dom.traversal.NodeFilter}.
   */
  ...
  /**
   * Initialize this node copy by setting the whatToShow property and the
  ...
  /**
   * True if this copy has a null namespace and a local name of {@link #WILD}.
   *  @serial
   */
  private boolean m_isTotallyWild;

  /**
   * Get the static score for this node copy.
  ...
  /**
   * Set the static score for this node copy.
  ...
  /**
   * Get the score that this copy will return if a copy succeeds.
   *
   *
   * @return the score that this copy will return if a copy succeeds.
   */
  ...
  /**
   * Tell what node type to copy, if not DTMFilter.SHOW_ALL.
   *
  ...
  /**
   * Tell what the copy score is for the given node.
   *
   *
          ...
          throws javax.xml.transform.TransformerException
  {
    ...
    // "A node copy * is true for any node of the principal node type.
    // For example, child::* will select all element children of the
    // context node, and attribute::* will select all attributes of
    // the context node."
    // "A node copy can have the form NCName:*. In this case, the prefix
    // is expanded in the same way as with a QName using the context
    // namespace declarations. The node copy will be true for any node
  ...
  /**
   * Test the current node to see if it matches the given node copy.
   *
   * @param xctxt XPath runtime context.
   *
   * @return {@link com.sun.org.apache.xpath.internal.patterns.NodeTest#SCORE_NODETEST},
