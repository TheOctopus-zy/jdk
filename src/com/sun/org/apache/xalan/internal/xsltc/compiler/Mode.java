...
final class Mode implements Constants {
    ...
    /**
     * A mapping between templates and copy sequences.
     */
    ...
    /**
     * Process all the copy patterns in this mode
     */
...
*/
        ...
        while (templates.hasMoreElements()) {
            ...
            // Add this template to a copy sequence if it has a pattern
    ...
    private void flattenAlternative(Pattern pattern,
        ...
        // Finally we have a pattern that can be added to a copy sequence!
    ...
    /**
     * Complete copy sequences of a given type by adding all patterns
     * from a given group.
     */
    ...
    /**
     * Build copy sequences. The first step is to complete the copy sequences
     * by including patterns of "*" and "node()" kernel to all element copy
     * sequences, and of "@*" to all attribute copy sequences.
     */
    private void prepareTestSequences() {
        final Vector starGroup = _patternGroups[DTM.ELEMENT_NODE];
        final Vector atStarGroup = _patternGroups[DTM.ATTRIBUTE_NODE];

        // Complete copy sequence for "text()" with "child::node()"
        completeTestSequences(DTM.TEXT_NODE, _childNodeGroup);

        // Complete copy sequence for "*" with "child::node()"
        completeTestSequences(DTM.ELEMENT_NODE, _childNodeGroup);

        // Complete copy sequence for "pi()" with "child::node()"
        completeTestSequences(DTM.PROCESSING_INSTRUCTION_NODE, _childNodeGroup);

        // Complete copy sequence for "comment()" with "child::node()"
        completeTestSequences(DTM.COMMENT_NODE, _childNodeGroup);

        // Complete copy sequence for "@*" with "attribute::node()"
            ...
            _childNodeGroup != null || _attribNodeGroup != null)
        {
            ...
            // Complete copy sequence for user-defined types
            for (int i = DTM.NTYPES; i < n; i++) {
                ...
                if (isAttributeName(name)) {
                    // If an attribute then copy "@*" to its copy sequence
                    completeTestSequences(i, atStarGroup);

                    // And also copy "attribute::node()" to its copy sequence
                    completeTestSequences(i, _attribNodeGroup);
                }
                else {
                    // If an element then copy "*" to its copy sequence
                    completeTestSequences(i, starGroup);

                    // And also copy "child::node()" to its copy sequence
...
*/
    ...
    private InstructionList compileNamespaces(ClassGenerator classGen,
        ...
        // Append switch() statement - namespace copy dispatch loop
        ...
        if (namespaceCount > 0) {
            ...
            // Add copy sequences for known namespace types
            ...
            // Return "null" if no copy sequences were compiled
   ...
   /**
    ...
    public void compileApplyTemplates(ClassGenerator classGen) {
        ...
        // Now compile copy sequences for various match patterns:
        for (int i = DTM.NTYPES; i < targets.length; i++) {
            final TestSeq testSeq = _testSeq[i];
            // Jump straight to namespace tests ?
